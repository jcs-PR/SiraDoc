...
function prepareSite(iPath, dest) {
...
function buildHtmlFiles(tree, src, dest) {
  for (let index = 0; index < tree.length; ++index) {
    ...
    if (pathObj.type === 'file') {
      ...
      let markdown = fs.readFileSync(srcFile, 'utf8');
...
function buildDocs(iPath, oPath) {
  ...
  let oPathManual = path.join(oPath, 'Manual', 'doc');
  let oPathScriptRef = path.join(oPath, 'ScriptReference', 'api');
...
function build(iPath, oPath) {
  ...
  prepareSite(iPath, dest);  // copy template over
  buildDocs(iPath, dest);
}
